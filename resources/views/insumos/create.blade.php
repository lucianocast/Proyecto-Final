...
<div class="max-w-3xl mx-auto py-6">
    <h2 class="text-lg font-semibold mb-4">Crear Nuevo Insumo</h2>
    ...
    <form action="{{ route('encargado.insumos.store') }}" method="POST">
        @csrf
        <div class="mb-2">
            ...
            <div class="flex gap-2">
                <select name="categoria_insumo_id" class="flex-1 border rounded px-2 py-1">
                    @foreach($categorias as $c)
                        <option value="{{ $c->id }}" {{ old('categoria_insumo_id') == $c->id ? 'selected' : '' }}>{{ $c->nombre }}</option>
                    @endforeach
                </select>
        ...
        <div class="mb-2">
            ...
            <input name="unidad_de_medida" value="{{ old('unidad_de_medida') }}" class="w-full border rounded px-2 py-1" placeholder="ej: g, ml, un" />
        </div>

        <div class="mb-2">
            <label class="block">Stock mínimo</label>
            <input name="stock_minimo" type="number" step="0.01" value="{{ old('stock_minimo', 0) }}" class="w-full border rounded px-2 py-1" />
        </div>

        <div class="mb-2">
            <label class="block">Ubicación</label>
            <input name="ubicacion" value="{{ old('ubicacion') }}" class="w-full border rounded px-2 py-1" />
        </div>
    ...
    <script>
        (function(){
            ...
            // If user clicks the 'Nueva categoría' link, open modal instead of navigating
            if (newBtn) {
